...
- `services.pgbackrest` now writes a log file by default (`log-level-file` defaults to `info` instead of `off`, in {file}`/var/log/pgbackrest`) and keeps its lock files in {file}`/run/pgbackrest` instead of {file}`/tmp`, so that the pgbackrest and the postgres user share them.
